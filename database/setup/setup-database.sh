...
# Load utilities for logging functions
...
setup_database() {
    ...
    psql -h $DB_HOST -p $DB_PORT -U $POSTGRES_USER -c "
        ...
        \$\$;
    " 2>/dev/null || {
        log_warn "Could not create user with $POSTGRES_USER, trying alternative method..."
        
        # Try creating database directly with current user
        if createdb $DB_NAME 2>/dev/null; then
            log_info "Database created with current user"
            return 0
        else
    ...
    " 2>/dev/null || {
        log_warn "Could not create database with $POSTGRES_USER, database may already exist"
    }
    ...
    " 2>/dev/null || {
        log_warn "Could not grant privileges, may already be set"
    }
}
...
EOF
        ...
        log_warn "Please update the API keys in .env file"
    else
