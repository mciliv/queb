...
app.get('/api/project-structure', async (req, res) => {
  try {
    const targetDir = resolveDir(req.query.dir || __dirname);
...
app.post('/api/cursorignore', async (req, res) => {
  try {
    const { paths, action, dir } = req.body; // action: 'add' or 'remove'
    const targetDir = resolveDir(dir || __dirname);
...
app.get('/api/cursorignore', async (req, res) => {
  try {
    const targetDir = resolveDir(req.query.dir || __dirname);
    const cursorignorePath = path.join(targetDir, '.cursorignore');
    let content = '';
