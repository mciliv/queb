...
export const findElement = (spec) => {
  ...
  if (Number.isFinite(maybeNumber)) {
    return ELEMENTS.find((element) => element.atomicNumber === maybeNumber) || null;
  }

  const normalized = normalizeKey(trimmed);
  const alias = ELEMENT_ALIASES[normalized] || normalized;
  return (
    ELEMENTS.find((element) => normalizeKey(element.name) === alias) ||
    ELEMENTS.find((element) => normalizeKey(element.symbol) === alias) ||
    null
  );
};

export const ELEMENT_LIST = ELEMENTS.map((element) => ({
  ...element,
