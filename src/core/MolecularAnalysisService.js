 ...
 * 
 * This is a deep module that provides a simple interface for molecular analysis
 ...
 * This service reduces complexity by providing a unified interface for all molecular analysis tasks.
 */
...
class MolecularAnalysisService {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!this.aiClient) {
        const { createClient } = require('../server/ai/openai/client');
      ...
      const openaiKey = configuration.get('openai.apiKey');
      if (this.aiClient && openaiKey) {
        this._capabilities.add('ai_analysis');
        this._capabilities.add('image_analysis');
      }
      ...
      this.logger.info('✅ Molecular Analysis Service initialized');
      
    } catch (error) {
      this.logger.error('❌ Failed to initialize Molecular Analysis Service:', error);
      throw new Error('Service initialization failed');
    }
  }

  /**
   * Analyze text description to extract molecular information
   * Simple interface hiding complex AI interaction and data processing
   */
  async analyzeText(description, options = {}) {
    ...
    if (!this._capabilities.has('ai_analysis')) {
      throw new Error('AI analysis not available - check API key configuration');
    }
    ...
    try {
      ...
      this.logger.info(`✅ Analysis complete: Found ${withStructures.length} molecules`);
      ...
      throw this._createAnalysisError(error, 'text_analysis');
    }
  }

  /**
   * Analyze image to extract molecular information
   * Simple interface hiding complex image processing and AI vision
   */
  async analyzeImage(imageData, coordinates = null, options = {}) {
    await this.initialize();

    if (!this._capabilities.has('image_analysis')) {
      throw new Error('Image analysis not available - check AI service configuration');
    }

    try {
      this.logger.info('🖼️ Analyzing image for molecular content');

      // First, detect the main object in the image
      const detection = await this._detectObject(imageData, coordinates);
      
      // Then analyze the detected object for molecular content
      if (detection.object && detection.object !== 'Unknown object') {
        const textResult = await this.analyzeText(detection.object, {
          ...options,
      ...
      return {
        ...
        reason: 'Could not identify a specific object for molecular analysis',
        molecules: [],
        imageAnalysis: detection
      };

    } catch (error) {
      this.logger.error('❌ Image analysis failed:', error);
      throw this._createAnalysisError(error, 'image_analysis');
    }
  }
  ...
  async generateStructures(smilesArray, options = {}) {
    ...
    try {
      ...
      throw this._createAnalysisError(error, 'structure_generation');
    }
  }
  ...
  async _callAI(params) {
    const model = configuration.get('openai.model');
    const timeout = configuration.get('openai.timeout');
   ...
   */
  _createAnalysisError(originalError, context) {
    const error = new Error(`Molecular analysis failed: ${originalError.message}`);
...
const molecularAnalysisService = new MolecularAnalysisService();

module.exports = molecularAnalysisService;
